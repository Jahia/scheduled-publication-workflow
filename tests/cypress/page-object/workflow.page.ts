...
class WorkflowPage extends BasePage {
    ...
    openWorkflowAsEditorInChiefAndVerifyButtons(scheduled: boolean, abort: boolean) {
        ...
        cy.get(WorkflowPage.SCHEDULED_DATE_INPUT).should(this.BE_VISIBLE).clear({force: true})
        ...
        if (abort) {
            ...
            cy.get('#workflow-dashboard-publication-window').contains('button', actionButtonLabel).should(this.BE_VISIBLE).click();
        }
    }
